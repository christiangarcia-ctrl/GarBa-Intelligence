(() => {
  ...
  function projection(extraMonthly = 0) {
    const years = Math.max(1, num(state.retirementAge) - num(state.age)); const months = years * 12;
  ...
  function runAnalysis() {
    ...
    const phrases=['Organizando lo que ya has construido…','Proyectando tus recursos…','Comparando tu avance con la vida que deseas…','Tu Ruta está lista.']; let i=0;
    byId('analysisText').textContent=phrases[0]; const timer=setInterval(()=>{i++; if(i<phrases.length){byId('analysisText').textContent=phrases[i];} else {clearInterval(timer);setTimeout(renderResults,350);}},560);
  }
  ...
  function renderResults() {
    ...
    animateNumber(byId('totalPatrimony'),r.totalPatrimony); animateNumber(byId('estimatedIncome'),r.monthlyIncome); animateNumber(byId('goalResult'),goal);
    animateNumber(byId('coverageResult'),coverage,v=>`${Math.round(v)}%`,850); byId('yearsResult').textContent=`${r.years} años`;
    const coverageLabel=coverage>=100?'Meta cubierta':coverage>=75?'Buen avance':coverage>=50?'En construcción':'Oportunidad importante'; byId('coverageLabel').textContent=coverageLabel;
    byId('incomeNarrative').textContent=`Con los supuestos actuales, tus recursos podrían representar alrededor de ${money.format(r.monthlyIncome)} mensuales en dinero de hoy. Es una estimación, no una garantía.`;
  ...
  function updateScenario() {
    const extra=num(byId('scenarioContribution').value); const base=projection(0); const scenario=projection(extra);
    byId('scenarioContributionLabel').textContent=money.format(extra); byId('scenarioIncomeComparison').textContent=money.format(scenario.monthlyIncome);
    const diff=scenario.monthlyIncome-base.monthlyIncome; byId('scenarioImpact').textContent=diff>0?`Podría representar ${money.format(diff)} adicionales al mes.`:'Sin cambios todavía';
  }
  byId('scenarioContribution').addEventListener('input',updateScenario); byId('resetScenario').addEventListener('click',()=>{byId('scenarioContribution').value=0;updateScenario();});
