(() => {
  ...
  const panels={
    ...
    manageData:{kicker:'Tu información',title:'Administrar mi información',body:'<p>Tu información permanece únicamente en este dispositivo.</p><button type="button" class="primary-action" id="clearRetirementData">Comenzar una nueva Ruta</button><p style="margin-top:12px">Esta acción elimina únicamente los datos de Mi Ruta de Retiro. Tu acceso y las demás herramientas permanecen sin cambios.</p>'}
  };
  ...
  document.addEventListener('click',(event)=>{
    if(event.target.id==='clearRetirementData'){
      const ok=window.confirm('¿Deseas comenzar una nueva Ruta? Se eliminarán únicamente los datos de esta herramienta.');
      if(!ok)return;
